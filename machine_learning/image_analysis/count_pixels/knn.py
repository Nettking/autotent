...
def count_leaves(image_path):
    ...
    for color, (lower, upper) in hsv_ranges.items():
        ...
        # Find pixels closest to target pixels
        closest_pixels = find_pixels_closest_to_target(image_bgr, [(255, 0, 0), (0, 255, 0), (0, 0, 255)])  # Example: Red, Green, Blue
        print(f"Closest pixels to target pixels in image {image_path}:")
...
def find_pixels_closest_to_target(image, target_pixels):
    closest_pixels = []

    # Iterate through each target pixel
    for target_pixel in target_pixels:
        ...
        for y in range(image.shape[0]):
            for x in range(image.shape[1]):
                # Calculate Euclidean distance between target pixel and current pixel
                distance = np.linalg.norm(image[y, x] - target_pixel)

                # Update closest pixel if distance is smaller
                if distance < min_distance:
                    min_distance = distance
                    closest_pixel = (x, y)

        closest_pixels.append(closest_pixel)
