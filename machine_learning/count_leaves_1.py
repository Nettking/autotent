...
def count_leaves(image_path):
    ...
    # Apply morphological operations to the green mask
    kernel = np.ones((7, 7), np.uint8)
    closing = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel, iterations=2)
    opening = cv2.morphologyEx(closing, cv2.MORPH_OPEN, kernel, iterations=2)

    # Use Canny edge detection on the original image
    edges = cv2.Canny(image_bgr, 50, 150)

    # Combine the green mask and edges
    combined_mask = cv2.bitwise_or(opening, edges)
